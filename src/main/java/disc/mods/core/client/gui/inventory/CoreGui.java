	...
	public void drawLiquidBar(TileEntity tank, int xOffset, int yOffset) {
		...
		if (tank != null) {
			if (tank instanceof IFluidHandler) {
				 ...
				 * BlockOrientationHelper.getOrientation(meta); FluidTankInfo[] info =
				 * ((IFluidHandler) tank).getTankInfo(facingDir); if (info != null &&
				 * info.length > 0) { FluidTankInfo tankInfo = info[0]; int tankCapacity =
				 * tankInfo.capacity; if (tankInfo.fluid != null && tankInfo.fluid.getFluid() !=
				 * null) { Fluid fluid = tankInfo.fluid.getFluid(); IIcon fluidIcon =
				 * fluid.getIcon(); int amount = tankInfo.fluid.amount; int fluidHeight =
				 * MathHelper.ceiling_float_int((amount / (float) tankCapacity) * 48);
				 * this.mc.getTextureManager()
				 * .bindTexture(this.mc.getTextureManager().getResourceLocation(
				 * fluid.getSpriteNumber())); this.drawTexturedModelRectFromIcon(startX + 1,
				 * startY + 1, fluidIcon, 16, 16); this.drawTexturedModelRectFromIcon(startX +
				 * 1, startY + 1 + 16, fluidIcon, 16, 16);
				 * this.drawTexturedModelRectFromIcon(startX + 1, startY + 1 + 32, fluidIcon,
				 * 16, 16); this.mc.getTextureManager().bindTexture(Textures.Gui. LiquidBar);
